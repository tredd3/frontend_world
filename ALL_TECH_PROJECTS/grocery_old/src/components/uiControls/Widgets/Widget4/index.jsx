...
class Widget4 extends React.Component {
    ...
    render() {
        ...
        if (data.length) {
            return (
                <HorizontalSlider className={classes.Widget4Wrapper} >
                    {data.map((product, index) => (
                        <div className={classes.Widget4} key={index} onClick={this.clickHandler({ type, subType, bannerId: product.bannerId })}>
                            <LazyLoadImage
                                alt="image"
                                height={"auto"}
                                effect="blur"
