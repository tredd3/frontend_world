...
describe('User profile addresses', () => {
  ...
  it('should behave as expected when there\'s no store associated with the address', async () => {
    ...
    expect(addressEditIcon.parentNode.href).toEndWith('/account/addresses/123123/map');

    expect(getByText('+ Add kirana partner')).toBeInTheDocument();
  });

  // eslint-disable-next-line max-len
  it('should behave as expected when there\'s a store associated with the address, which matches the user\'s preferred address', async () => {
    ...
    expect(kiranaEditIcon.parentNode.href).toEndWith('/account/addresses/123123/select-kirana');
